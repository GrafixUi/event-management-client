...
const Venuelist2 = () => {
  ...
  <div className="py-5 mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 max-sm:grid-cols-2 max-sm:gap-4">
    {items.map((item) => (
      <Link key={item.id} to={`/movieselection?movieid=${item.id}`}>
        <div className="bg-white shadow-sm shadow-slate-300 flex flex-col items-stretch pb-6 rounded-2xl mb-5 justify-center transition duration-500 hover:scale-105 max-w-[300px] max-sm:w-[170px] max-sm:-ml-2">
          <img
            loading="lazy"
            srcSet={product1}
            alt='product'
          ...
          <div className="self-center flex justify-between gap-5 mt-0.5 items-start w-full">
            ...
            <div className="self-stretch flex-grow flex-col items-stretch px-5 max-sm:w-32">

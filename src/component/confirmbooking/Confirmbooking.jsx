...
const Confirmbooking = () => {
  ...
  const movieDetails = useStore((state) => state.movieOrderDetails);
  ...
  const movieqrvalue = `Movie Name: ${movieDetails?.movietitle} \n Quantity: ${movieDetails?.selectedSeatsName} \n Price: ${movieDetails?.totalprice} \n Theatre: ${movieDetails?.theater} \n Address: ${movieDetails?.address}`;
  if (!type) {
    navigate("/");
  }
                      ...
                      <div className="flex flex-col items-stretch w-[51%] max-md:w-full max-md:ml-0">
                        <div className="flex flex-col items-stretch my-auto max-md:max-w-full max-md:mt-10">
                          ...
                          <h2 className="text-white text-xl font-medium leading-8 whitespace-nowrap mt-1.5 self-start">
                            {orderDetails?.eventDetails?.day}{" "}
                            {orderDetails?.eventDetails?.month}{" "}
                            {orderDetails?.eventDetails?.year}
                          </h2>
                    ...
                    <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
                      <div className="flex flex-col items-stretch w-[51%] max-md:w-full max-md:ml-0">
                        <div className="flex flex-col items-stretch my-auto max-md:max-w-full max-md:mt-10">
                          ...
                          <h2 className="text-white text-xl font-medium leading-8 whitespace-nowrap mt-1.5 self-start">
                            {movieDetails?.day} {movieDetails?.month}{" "}
                            {movieDetails?.year}
                          </h2>
                          <h3 className="text-neutral-400 text-lg leading-7 mt-5">
                            Ticket :{movieDetails?.selectedSeatsName}
                            {/* {movieDetails?.ticketdetails.map((ticket) => {
                              if (ticket.attributes.quantity > 0) {
                                return (
                                  <div>
                                    {ticket.attributes.tickettitle} -{" "}
